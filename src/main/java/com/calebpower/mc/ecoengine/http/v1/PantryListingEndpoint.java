...
public class PantryListingEndpoint extends JSONEndpoint {

  /**
   * Facilitates the retrieval of a cookbook's pantry.
   *
   * @author Caleb L. Power <cpower@axonibyte.com>
   */
  public PantryListingEndpoint() {
    super("/cookbooks/:cookbook/pantry", APIVersion.VERSION_1, HTTPMethod.GET);
  }
  ...
  @Override public JSONObject doEndpointTask(Request req, Response res) throws EndpointException {
    try {
      ...
      Set<Commodity> pantry = new HashSet<>();
      for(var commodity : cookbook.getPantry()) {
        pantry.add(
            Database.getInstance().getCommodity(commodity));
      }

      res.status(200);
      return new JSONObject()
          .put("status", "ok")
          .put("info", "Retrieved pantry.")
          .put("cookbook", cookbook.getID().toString())
          .put("pantry", (JSONArray)pantry
              .stream()
